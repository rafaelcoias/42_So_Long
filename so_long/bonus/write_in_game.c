...
static void	write_score_number(t_game *game, int width, int nbr)
{
	...
	{
		write_score_number(game, width - 15, nbr / 10);
		write_score_number(game, width, nbr % 10);
	}
}
...
void	write_score(t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx, game->window, game->img.re,
        game->width - 150, 0);
	write_score_number(game, game->width - 40, game->count_moves);
}
